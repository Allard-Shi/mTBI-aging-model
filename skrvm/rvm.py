...
class RVR(BaseRVM, RegressorMixin):
    ...
    def get_weight(self, params_print = False):
        """Return the RVR model parameters.""" 
        if params_print == True:
            print('='*10+' params '+ '='*10)
            print("Alpha: {}".format(self.alpha_.shape))
            print("Beta: {}".format(self.beta_))
            print("Gamma: {}".format(self.gamma.shape))
            print("Sigma: {}".format(self.sigma_.shape))
            print("mean weight: {}".format(self.m_.shape))
            print("Relevance Vectors: {}".format(self.relevance_.shape))
        return self.alpha_, self.beta_, self.gamma, self.sigma_, self.m_, self.relevance_,

    def predict(self, X, eval_MSE=False, print_relevance = False, print_phi = False):
        """Evaluate the RVR model at x."""
        phi = self._apply_kernel(X, self.relevance_)
        
        if print_relevance:
            print('Relevance:',self.relevance_.shape)
        ...
        if eval_MSE:
            MSE = np.diagonal(np.dot(phi, np.dot(self.sigma_, phi.T)))
            return y, MSE
        else:
            return y
